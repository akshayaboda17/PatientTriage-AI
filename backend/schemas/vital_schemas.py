...
class VitalSignInput(BaseModel):
    hr: Optional[int] = Field(None, ge=0, le=300, description="Heart rate in bpm")
    sbp: Optional[int] = Field(None, ge=0, le=350, description="Systolic blood pressure in mmHg")
    dbp: Optional[int] = Field(None, ge=0, le=250, description="Diastolic blood pressure in mmHg")
    rr: Optional[int] = Field(None, ge=0, le=100, description="Respiratory rate in breaths/min")
    spo2: Optional[int] = Field(None, ge=0, le=100, description="SpO2 oxygen saturation percentage")
    temp: Optional[float] = Field(None, ge=20.0, le=50.0, description="Temperature in Celsius")
