...
import { 
  ArrowLeft, Activity, ShieldAlert, PlusCircle, CheckCircle2, Stethoscope, RefreshCw, X, Edit3, Sparkles, LogOut
} from 'lucide-react';
...
export const PatientDetailView = ({ encounterId, onBack, onOpenReview, onAlertStateChanged }) => {
  ...
  const handleDischargePatient = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        addToast("Patient successfully discharged from ED. Care space released.", "success");
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/90 border border-slate-800 p-5 rounded-2xl shadow-xl">
        ...
        <div className="flex items-center gap-2">
          ...
          {hasPermission('patient:update') && encounter.status !== 'DISCHARGED' && (
            <button
              onClick={() => setShowDischargeModal(true)}
              className="flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-rose-600/20 hover:bg-rose-600 text-rose-300 hover:text-white border border-rose-500/30 text-xs font-bold transition-all cursor-pointer"
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-5">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base font-bold text-white tracking-tight">Confirm Patient Discharge</h3>
                  <p className="text-xs text-slate-400">Release assigned care bed and conclude ED encounter</p>
                </div>
              </div>
              <button
            ...
            <div className="p-3.5 bg-slate-950/70 rounded-2xl border border-slate-800/80 space-y-1 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-white">{patient?.first_name} {patient?.last_name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Encounter ID:</span>
                <span className="font-mono text-cyan-300">{encounter?.encounter_id}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Assigned Care Space:</span>
                <span className="font-mono text-amber-300">{encounter?.bed_number ? `Bed ${encounter.bed_number}` : 'Waiting Area (Unbedded)'}</span>
              </div>
            </div>
