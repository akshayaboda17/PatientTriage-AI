  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-base font-bold text-white tracking-tight">Physician Review History & Audit Trail</h3>
      </div>
        ...
        {assessments.map((a, idx) => (
          <div key={a.id || idx} className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between text-xs">
              <div className="flex items-center gap-2">
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                  ...
                  {a.ai_agreement === 'OVERRIDDEN' ? 'AI OVERRIDDEN' : 'AGREED WITH AI'}
                </span>
                <span className="font-bold text-slate-200">{a.physician_name} ({a.physician_role})</span>
              </div>
            ...
            <div className="text-xs text-slate-300">
              <strong>Clinical Disposition:</strong> <span className="font-mono text-cyan-300">{a.clinical_decision}</span>
            </div>

            {a.override_reason && (
              <div className="text-xs text-amber-300 bg-amber-950/30 p-2.5 rounded-lg border border-amber-800/40">
                <strong>Override Rationale:</strong> {a.override_reason}
              </div>
            )}
